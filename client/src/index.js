...
const persistedReducer = persistReducer(persistConfig, authReducer); //this is the reducer we are going to use, we are going to use the authReducer we created in client\src\state\index.js  This creates a new reducer that wraps authReducer and adds persistence functionality to it. This ensures that the state managed by the authReducer is now persisted using the Redux Persist functionality.
...
}); // This allows the state to be stored and retrieved from the specified storage (in this case, local storage) across page reloads or app restarts.
